...
Le [Ministère de l’Action et des Comptes](https://www.gouvernement.fr/le-ministere-de-l-action-et-des-comptes-publics) publics souhaite développer des outils génériques et auto-apprenants de lutte contre la fraude, s'appuyant sur des méthodes avancées de fouille de données et d'analyse de réseaux, qui permettront de rapprocher des sources de données diverses et de détecter des schémas de fraude connus ou nouveaux.
...
Face à cet impératif économique, des systèmes évolués de détection de fraude émergent, traitant une abondance de données hétérogènes, visualisant ces données sous forme de graphe et élaborant des modèles prédictifs ou de détection d'anomalies. En parallèle, inspirées par les systèmes d’informations d’entreprise ou du net, les modélisations sémantiques d’entités et de leurs liens -ou ontologies- permettent la recherche ou l’apprentissage à un niveau d’abstraction élevé (cf. Knowledge Graph -ou graphe de connaissance- de Google) et donnent la possibilité à des intelligences artificielles de raisonner sur ces entités. 
...
* **développer des méthodes de détection** d’individus, d'organisations ou de communautés d’intérêt à partir du graphe.

Capitalisant sur des travaux existants, la mission aura comme point de départ une première approche de construction du graphe de connaissance, ainsi que des outils de visualisation et d’analyse de graphes / réseaux.
...
* **faire évoluer les outils de visualisation et de détection de fraude** pour exploiter au mieux le modèle sémantique et la richesse du graphe. On pourra éventuellement investiguer le potentiel d’approches à l’état de l’art telles que les graph embeddings, le deep learning (e.g. autoencodeurs), etc.
...
- exploration du graphe et détection de fraude.
...
**Missions principales** : faciliter l'intégration des données et leur visualisation ; développer des outils pour les analystes :
...
**Compétences nécessaires** : résolution d'entités, visualisation de données, développement web.
